...
game_name = 'car_racing_dueling_dqn'    # the name of the game being played for log files
...
class DQN_agent:
    ...
    def build_model(self, network_name):
        with tf.variable_scope(network_name):
            ...
            h_fc15_state = tf.layers.dense(model, 512, activation=tf.nn.relu)
            h_fc15_action = tf.layers.dense(model, 512, activation=tf.nn.relu)
            
            h_fc16_state = tf.layers.dense(h_fc15_state, self.action_size)
            h_fc16_action = tf.layers.dense(h_fc15_action, self.action_size)
